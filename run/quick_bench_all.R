# Quick benchmark: C++ Gram-CD vs glmnet vs EGLearn
...
cat("=== Benchmark: C++ Gram-CD vs glmnet vs EGLearn (5 reps) ===\n\n")
...
for (ci in seq_along(configs)) {
  ...
  for (rep in seq_len(nsim)) {
    ...
    if (!is.null(path_cpp)) {
      ...
      best_f1_cpp <- max(f1_cpp, na.rm = TRUE)
    } else {
      best_f1_cpp <- NA
    }
    ...
    cat(sprintf("  rep %d: cpp=%.1fs (F1=%.3f) | glm=%.1fs (F1=%.3f) | EGL=%.1fs (F1=%.3f)\n",
...
agg$ratio_cpp <- agg$time_eg / agg$time_cpp
agg$ratio_glm <- agg$time_eg / agg$time_glm
print(agg, digits = 3)
